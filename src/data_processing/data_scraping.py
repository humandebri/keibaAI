...
class RaceScraper:
    ...
    def save_data(self, df: pd.DataFrame, year: int, format: str = 'xlsx') -> str:
        """データを保存"""
        output_path = os.path.join(self.output_dir, f'{year}.{format}')
...
def main():
    """メイン関数"""
    parser = argparse.ArgumentParser(description='競馬データスクレイピング')
    ...
    scraper = RaceScraper(output_dir=args.output, max_workers=args.workers)
