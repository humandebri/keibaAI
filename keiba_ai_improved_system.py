#!/usr/bin/env python3
"""
競馬AI改良システム - オッズ依存を減らし、ROIを重視した実運用可能モデル
"""
...
class ImprovedKeibaAISystem:
    ...
    def _get_default_config(self) -> Dict:
        ...
        return {
            ...
            'betting': {
                ...
                'kelly_fraction': 0.1,      # Kelly基準の使用率
                'max_bet_fraction': 0.02    # 最大ベット比率
            }
        }
    ...
    def create_advanced_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 5. 馬体重変化の影響度
        if '体重変化' in df.columns and '体重_numeric' in result.columns:
        ...
        if '馬体重_numeric' in result.columns:
    ...
    def evaluate_calibration(self, y_true: np.ndarray, y_pred_proba: np.ndarray) -> Dict:
        """予測確率の校正評価"""
        # 10分割で実際の的中率と予測確率を比較
        ...
        for i, race_id in enumerate(unique_races[:2000]):
            ...
            for idx, (_, horse) in enumerate(race_data.iterrows()):
                if pred_ranks[idx] <= threshold_rank:
                    ...
                    if bet_amount < 100 or bet_amount > capital * 0.1:
                        continue
...
def main():
    """メイン実行関数"""
    print("=" * 60)
    print("競馬AI改良システム - ROI重視の実運用可能モデル")
    print("=" * 60)
    ...
    if results['roi'] > 1.0:
        ...
        print("ただし、実際の運用では取引コストやスリッページを考慮してください。")
        return True
    else:
        print("\n📊 ROI < 1.0 です。さらなる改善が必要です。")
        print("セグメント特化やより高度な特徴量エンジニアリングを検討してください。")
        return False
